...
const Card = ({ info, showInfo, secondClass }) => {
  ...
  const convertLease = (lease) => {
    if (lease >= 1000 && lease < 999999) return `${lease / 1000}k `;
    if (lease >= 1000000) return `${lease / 1000000}m `;
    return lease;
  };
    ...
    <div className={`${styles.card_container} ${secondClass}`}>
      ...
      <div className={styles.card_buy}>
        <div className={styles.prices}>
          ...
          <h2
            style={showInfo.lease ? {} : { display: 'none' }}
          >{`${convertLease(info.lease)} $`}</h2>
        </div>
